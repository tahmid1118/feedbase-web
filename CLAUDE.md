...
- **Logged-in users on the portal.** `PortalComments` and `PostOwnerActions` read `useSession()`; when logged in, a comment/post is sent with the Bearer token (backend attributes it to the user, showing their name + avatar) and the author can **edit/delete their own** post/comment via `lib/portal/actions.ts` (`portalActions`). Ownership is compared client-side (`comment.author_id === session.user.id`) for showing controls and **enforced server-side**. The session must reach the portal host: the auth cookie is scoped to the **root domain in production** (`auth.ts` `SESSION_COOKIE_DOMAIN`), so tenant subdomains share the login. In **dev** the cookie is host-only (a `localhost` Domain cookie is rejected by browsers), so test logged-in portal actions at `http://localhost:3000/portal/<tenant>` (not the `*.localhost` subdomain).
