...
- Local testing: visit `http://<tenant>.localhost:3000` (Chrome resolves `*.localhost`) or `/portal/<tenant>` directly. **To test *logged-in* portal actions, use `http://localhost:3000/portal/<tenant>`** — a `*.localhost` subdomain can't receive the login cookie in dev (see the logged-in-users note above).
